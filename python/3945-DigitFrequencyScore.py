'''
3945. Digit Frequency Score
Easy
Topics
premium lock icon
Companies
Hint
You are given an integer n.

The score of n is defined as the sum of d * freq(d) over all distinct digits d, where freq(d) denotes the number of times the digit d appears in n.

Return an integer denoting the score of n.

 

Example 1:

Input: n = 122

Output: 5

Explanation:

The digit 1 appears 1 time, contributing 1 * 1 = 1.
The digit 2 appears 2 times, contributing 2 * 2 = 4.
Thus, the score of n is 1 + 4 = 5.
Example 2:

Input: n = 101

Output: 2

Explanation:

The digit 0 appears 1 time, contributing 0 * 1 = 0.
The digit 1 appears 2 times, contributing 1 * 2 = 2.
Thus, the score of n is 2.
 

Constraints:

1 <= n <= 109
'''
# returns index of item if found, else -1
def searchMatrix(matrix: list[list[int]], num: int) -> int:
    i = 0
    while i < len(matrix):
        if len(matrix[i]) == 0:
            break
        if matrix[i][0] == num:
            print(i)
            return i
        i = i+1
    return -1
def frequencyOp(matrix: list[list[int]]) -> int:
    total = 0

    for item in matrix:
        total = total + item[0] * item[1]
    return total

class Solution:
    def digitFrequencyScore(self, n: int) -> int:
        matrix = []

        num = n
        while num != 0:
            print(num)
            current = num % 10
            num = num // 10

            index = searchMatrix(matrix, current)
            if index >= 0:
                matrix[index][1] = matrix[index][1] + 1
            else:
                matrix.append([current, 1])

        return frequencyOp(matrix)

def main():
    solution = Solution()
    print(solution.digitFrequencyScore(0))

main()
        
            



        